...
server.tool(
  "flowcase_list_consultants",
  "List all consultants from Flowcase with name, title, office, and CV ID",
  { countryCode: z.string().optional().default("no").describe("Country code to filter by (default: no)") },
  async ({ countryCode }) => {
...
server.tool(
  "flowcase_get_cv",
  "Get a consultant's full CV from Flowcase including title, qualifications, skills (with years), and all projects",
  {
  ...
  async ({ userId, cvId }) => {
    ...
    return { content: [{ type: "text" as const, text: formatCv(cv) }] };
  }
);

// --- Tool 3: Get projects only ---
server.tool(
  "flowcase_get_projects",
  "Get only the project experiences for a consultant (lighter than full CV)",
  {
    userId: z.string().describe("Flowcase user ID"),
    cvId: z.string().describe("CV ID (use default_cv_id from list_consultants)"),
  },
...
server.tool(
  "flowcase_search_by_skill",
  "Find consultants who have a specific skill in their Flowcase CV. Fetches all CVs and filters (may be slow on first call).",
  {
  ...
  async ({ skill, countryCode }) => {
    ...
    const matches: Array<{
      ...
      userId: string;
      office: string | null;
      matchedSkills: Array<{ name: string; years: number }>;
    }> = [];
...
server.tool(
  "flowcase_search",
  "Search for consultants by keyword across skills, project customers, project descriptions, and roles. Case-insensitive partial match.",
  {
      ...
      try {
        ...
        const matchedProjects = (cv.project_experiences ?? [])
          .map((p) => {
            ...
            if (customer.toLowerCase().includes(queryLower)) matchedIn.push("customer");
            if (description.toLowerCase().includes(queryLower)) matchedIn.push("title");
            if (longDesc.toLowerCase().includes(queryLower)) matchedIn.push("description");
            if (roleNames.toLowerCase().includes(queryLower)) matchedIn.push("role");
